...
fetch(`https://public-api.wordpress.com/rest/v1.1/sites/ahardknoxlife28994277.wordpress.com/posts/${post_id}`).then(function(data){
    data.json().then(function(data) {
        var postContainer = document.querySelector('.site-content'),
            htmlString = `
                ...
                <div class="post__social">
                    <a class="post__social__link" data-facebook="true" target="_blank" href="https://www.facebook.com/sharer.php?description=${data.title}&u=${encodeURIComponent(window.location.href)}&picture=${data.featured_image}">
        ...
        document.querySelector('.post__social__link[data-facebook=true]').addEventListener('click', (ev)=> {
            ev.preventDefault();
            FB.ui({
                ...
                action_properties: JSON.stringify({
                    object: {
                        ...
                        'og:description': data.excerpt.replace(/<[^>]*>/g, "")
                    }
                })
            },
            function (response) {
